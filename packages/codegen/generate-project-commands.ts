#!/usr/bin/env bun
/**
 * generate Project commands from extracted-schema.json
 *
 * input: packages/codegen/extracted-schema.json, packages/codegen/cli-spec.json
 * output: packages/cli/src/generated/project.ts
 *
 * generates:
...
const outputPath = join(outputDir, "project.ts");
...
const projectEntity = schema.entities.Project;
const projectCommand = cliSpec.commands.find(c => c.command === "project");
const projectsCommand = cliSpec.commands.find(c => c.command === "projects");

if (!projectCommand || !projectsCommand) {
  throw new Error("project/projects commands not found in cli-spec.json");
}
...
function getExpectedCliFlags(): Set<string> {
  ...
  for (const f of projectCommand!.flags) {
    flags.add(f.name);
  }
  return flags;
}

function generateListProjectsInputSchema(): string {
  return `export const listProjectsInput = z.object({
...
function generateProjectInputSchema(): string {
  const lines: string[] = [];
  ...
  const updateFields = projectEntity.updateInput.fields
...
${generateListProjectsInputSchema()}
...
const generatedSchema = generateProjectInputSchema();
...
console.log(`\nparity check:`);
console.log(`  cli-spec.json project flags: ${expectedFlags.size}`);
console.log(`  generated project flags: ${generatedFlags.size}`);
