...
test("release pin is independent of agreement between local artifacts", async () => {
  const sdl = await readFile(new URL("./sdk-schema.graphql", import.meta.url), "utf8");
  const provenance = JSON.parse(await readFile(new URL("./sdk-schema.provenance.json", import.meta.url), "utf8"));
